      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
        ...
        <div className="max-w-md mx-auto">
          <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-8">
            ...
            {/* Monitor Students Button */}
            <Link
              href={`/admin/feedback/monitor${selectedSemester && selectedCourse ? `?semester=${selectedSemester}&course=${encodeURIComponent(selectedCourse)}${selectedBatch ? `&batch=${encodeURIComponent(selectedBatch)}` : ''}` : ''}`}
              className="w-full mt-3 py-3 rounded-xl text-sm font-semibold bg-white border-2 border-gray-900 text-gray-900 hover:bg-gray-50 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
              </svg>
              Monitor Students {selectedSemester && selectedCourse ? '(Filtered)' : '(All)'}
            </Link>
          </div>
